...
const BajaEmpleados = () => {
  ...
  const buscarEmpleadoPorDocumento = async () => {
    ...
    try {
      ...
      const response = await axios.get(
        "http://localhost:8080/empleados/obtener/documento/activo/" +
          nroDocumento
      );
  ...
  return (
    <div className="baja-container">
      ...
      <button className="volver-emp" onClick={() => navigate("/empleados")}>
        ← Volver
      </button>
      ...
      <div className="baja-form">
          ...
          <div className="campo">
            ...
            <input
              ...
              onKeyDown={(e) => {
                if (e.key === "Enter") {
                  e.preventDefault();
                  buscarEmpleadoPorDocumento(); // ← ejecutá la función
                }
              }}
        ...
        <button className="baja-btn" onClick={handleDarDeBaja}>
          DAR DE BAJA
        </button>
      </div>
      <ToastContainer />
