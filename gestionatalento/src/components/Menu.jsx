...
const Menu = () => {
    ...
    try {
      if (searchType === "documento") {
        const response = await axios.get(
          "http://localhost:8080/empleados/obtener/documento/" + searchValue
        );
  ...
  const handleSelect = (empleadoBusqueda) => {
    ...
    setCoincidencias([]);
    setSearchValue(
      `${empleadoBusqueda.persona.nombres} ${empleadoBusqueda.persona.apellidos}`
    );
  };
  ...
  return (
    <div className="main-grid">
          ...
          {coincidencias.length > 1 && (
            <div className="dropdown-results">
              {coincidencias.map((empleadoBusqueda, index) => (
                ...
                >
                  {empleadoBusqueda.empleado.persona.nombres}{" "}
                  {empleadoBusqueda.empleado.persona.apellidos}. (Ingreso:{" "}
                  {empleadoBusqueda.empleado.fecIngreso})
                </li>
              ))}
            </div>
          )}
      ...
      <div className="top-right">
        <label
          style={{ fontSize: "30px", fontWeight: "bold", marginBottom: "50px" }}
        >
      ...
      <div className="bottom-left">
            ...
            <input
              ...
              value={
                empleadoSeleccionado?.fecIngreso
                  ? new Date(empleadoSeleccionado.fecIngreso)
                      .getDate()
                      .toString()
                      .padStart(2, "0") +
                    "/" +
                    (new Date(empleadoSeleccionado.fecIngreso).getMonth() + 1)
                      .toString()
                      .padStart(2, "0") +
                    "/" +
        ...
        <div className="data-row">
          <div className="data-group">
            <label style={{ fontSize: "23px", marginTop: "20px" }}>
              Área de Desempeño
            </label>
            <input
              style={{ borderRadius: "20px" }}
              ...
              disabled
              value={
                empleadoSeleccionado?.cargo?.departamento?.descripcion || ""
              }
            />
          </div>
          <div className="data-group">
            ...
            <input
              ...
              value={
                empleadoSeleccionado?.fecEgreso
                  ? new Date(empleadoSeleccionado.fecEgreso)
                      .getDate()
                      .toString()
                      .padStart(2, "0") +
                    "/" +
                    (new Date(empleadoSeleccionado.fecEgreso).getMonth() + 1)
                      .toString()
                      .padStart(2, "0") +
                    "/" +
      ...
      <div className="bottom-right">
        <label style={{ fontSize: "23px", fontWeight: "bold" }}>
          Comentarios
        </label>
        <div>
          <textarea
            ...
            value={
              empleadoSeleccionado?.poseeDiscapacidad === "S"
                ? `Discapacidad: ${
                    empleadoSeleccionado.descripcionDiscapacidad ||
                    "No especificado"
                  }`
                : "Sin discapacidad"
            }
